<template>
	<div>
		...
		<el-row class="iptimefilter">
			...
			<el-col :span="10">
				<el-input v-model="items.iptimefilter"  type="number" min="1"></el-input>
				<p>低于此时间(分钟)的ip将不会提取到</p>
			</el-col>
			<el-col :span="5">&nbsp;</el-col>
		</el-row>
		<el-row>
			...
			<el-col :span="12">
				...
				<div class="province" v-if="items.seen_province">
					<el-radio-group v-model="items.select_province" size="medium" @change="(value)=>selectCity(value)">
						<el-radio v-for="item in items.province" :label="item.province" :key="item.id">{{item.province}}</el-radio>
					</el-radio-group>
				</div>
				<div class="city" v-if="items.seen_city">
					<el-select v-model="items.select_province" placeholder="请选择省份" @change="(value)=>selectCity(value)">
						<el-option v-for="item in items.province" :label="item.province" :value="item.id" :key="item.id"></el-option>
					</el-select>
					<el-select v-model="items.select_city" placeholder="请选择城市">
						<el-option v-for="item in items.city" :label="item.name" :value="item.id" :key="item.id"></el-option>
					</el-select>
				</div>
			</el-col>
			<el-col :span="4">&nbsp;</el-col>
		...
		<el-row>
			...
			<el-col :span="12">
				<div class="result_type">
					<el-radio-group v-model="items.separator" size="medium">
...
<script>
	...
	export default {
		...
		methods: {
			...
			selectCity:function(value){
				for(var i = 0;i<this.items.province.length;i++){
					...
					if(value==item.id){
						if(item.city){
							this.items.city=item.city
							this.items.select_city = item.city[0].id
						}else{
							this.items.city=[]
			...
			createurl:function(){
				...
				var data={
					...
					proxy_type:this.items.proxys_type,
					...
					filtertime:this.items.iptimefilter,
					show_city:this.items.show_city,
					show_carrier:this.items.show_carrier,
					show_expire:this.items.show_expire
				}
				this.$http.post(this.$config.host+'http/yizhou_get_ip/createurl',this.$qs.stringify(data)).then(
					response => {
						/*如果有来源登录成功返回来源*/
			...
			getFree:function(){
				this.$http.get(this.$config.host+'http/get_ip/freeIp',{
						params: {
							'proxy_type':2
						},
						}).then(
		...
		created:function(){
			this.$http.get(this.$config.host+'yizhou/area/getarea').then(
				response => {
					let res = response.data
					this.items.province = res.data.data
					this.items.select_province = this.items.province[0].province
					this.items.city = res.data.data[0].city
					this.items.select_city = this.items.city[0].id
				});
				this.$http.get(this.$config.host+'http/index/getTimelongList',{params:{type:2}}).then(
					response => {
						let res = response.data
				...
				this.$http.get(this.$config.host+'http/yizhou_get_ip/info').then(
					response => {
						let res = response.data
