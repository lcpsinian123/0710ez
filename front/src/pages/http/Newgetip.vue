	...
	export default {
		...
		methods: {
			...
			createurl:function(){
				...
				this.$http.post(this.$config.host+'http/yizhou_get_ip_new/createurl',this.$qs.stringify(data)).then(
					response => {
						/*如果有来源登录成功返回来源*/
		...
		created:function(){
			this.$http.get(this.$config.host+'http/index/getTimelongList',{params:{type:3}}).then(
				response => {
			...
			this.$http.get(this.$config.host+'http/yizhou_get_ip_new/info').then(
				response => {
					let res = response.data
